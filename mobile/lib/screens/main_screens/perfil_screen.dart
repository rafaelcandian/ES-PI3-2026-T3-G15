...
/* Hub de Identidade e Configurações de Segurança (User Control Center).
   Gerencia a sincronização de PII (Personally Identifiable Information) e orquestra 
   o ciclo de vida do Segundo Fator de Autenticação (Provisionamento, Ativação e Revogação). */
...
class _PerfilPageState extends State<PerfilPage> {
  ...
  /* Serviço usado nos fluxos de autenticação avançada, como 2FA. */
  final AuthService _authService = AuthService();

  /* Evita exibir dados incompletos antes do retorno do Firebase. */


  bool _loading = true;

  /* Valores iniciais evitam quebra se algum campo ainda não existir. */
  ...
  /* Controla o estado visual e funcional da autenticação em dois fatores. */
  ...
  void initState() {
    ...
    /* Carrega o perfil assim que a tela é aberta. */
  ...
  /* Motor de Sincronização de Perfil (Data Marshalling).
     Agrega dados do Firebase ID Token (E-mail verificado) com atributos extendidos do 
     Firestore (CPF, Telefone, Flags de 2FA). Implementa 'Lazy Loading' para carregar 
     informações sensíveis apenas sob demanda, otimizando o consumo de banda. */
  Future<void> _carregarDadosUsuario() async {
    ...
    try {
      ...
      /* Se não houver usuário logado, encerra sem quebrar a tela. */
      if (user == null) {
      ...
      /* Consulta dados extras do perfil no documento do usuário. */
      ...
      /* Mantém fallback do Auth caso falte algum campo no Firestore. */
      if (data != null) {
  ...
  /* Oculta parte do CPF por ser uma informação sensível. */
  ...
  /* Oculta parte do telefone sem impedir identificação pelo usuário. */
  ...
  /* Centraliza mensagens rápidas mantendo o padrão visual do app. */
  ...
  /* Abre a edição em modal para manter o usuário na mesma tela. */
  ...
  /* Atualiza Auth e Firestore para manter os dados sincronizados. */
  Future<void> _salvarPerfil({
    ...
    /* Salva somente números para facilitar validações e consultas. */
  ...
  /* Envia redefinição pelo Firebase, sem manipular senha no app. */
  ...
  /* Mostra retorno claro para sucesso ou falha no envio do e-mail. */
  ...
  /* Fluxo sinalizado sem exclusão automática para evitar remoção acidental. */
  ...
  /* Escolhe ativar ou desativar o 2FA conforme o estado atual. */
  ...
  /* Desativa o 2FA somente após confirmação do usuário. */
  ...
  /* Protocolo de Ativação 2FA (MFA Enrolment Workflow).
     1. Invoca createTwoFactorSetup para gerar semente criptográfica Base32.
     2. Abre Modal de Prova de Posse (Proof of Possession) para validar sincronia de clock.
     3. Apenas após a primeira validação bem-sucedida, o status no Firestore é promovido a 'enabled'. */
  ...
  /* Confirma o logout para evitar encerramento de sessão sem querer. */
...
/* Fundo decorativo que reforça a identidade visual premium da tela. */
...
/* Texto curto de contexto para orientar o usuário no perfil. */
...
/* Card que reúne dados pessoais e acesso rápido à edição. */
...
/* Linha reutilizável para manter e-mail, telefone e CPF padronizados. */
...
/* Agrupa recursos de segurança: autenticação, senha e 2FA. */
...
/* Separa ações sensíveis, como logout e exclusão de conta. */
...
/* Card base para manter espaçamento e bordas consistentes. */
...
/* Linha clicável usada em ações que abrem fluxos de segurança. */
...
/* Chave visual sincronizada com o estado do 2FA. */
...
/* Mostra status de segurança sem exigir ação do usuário. */
...
/* Campo reutilizável para manter padrão nos formulários. */
...
/* Formata o CPF durante a digitação e limita a 11 números. */
...
/* Formata telefone celular brasileiro durante a digitação. */
...
/* Bottom sheet para editar o perfil sem sair da página atual. */
...
class _EditProfileSheetState extends State<_EditProfileSheet> {
  ...
  /* Garante celular com DDD e 11 dígitos. */
  ...
  /* Valida o formulário antes de enviar alterações ao Firebase. */
...
/* Bottom sheet responsável pela confirmação do código autenticador. */
...
class _TwoFactorSetupSheetState extends State<_TwoFactorSetupSheet> {
  ...
  /* Aceita apenas códigos de 6 dígitos antes de chamar o serviço. */
  ...
  /* Facilita o cadastro manual da chave no app autenticador. */
  void _copiarChave() {
...
/* Estrutura base dos modais inferiores usados nesta tela. */
